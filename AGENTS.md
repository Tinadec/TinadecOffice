...
**Last Updated By:** Codex (documented the canonical operation/execution runtime contract and added the durable event replay/follow SSE contract)
...
## OVERVIEW
TinadecOffice is a Windows-first intelligent agent desktop workbench built around a universal agent harness. The target product combines a .NET 10 Core state/runtime service, an Elysia TypeScript Gateway, and an Electron + Vue desktop UI under one npm workspace root. The Core has been rebuilt as a MAF-based modular monolith; storage and read paths are in place, a legacy DmaEA dual-layer invocation exists, and a TOML operation/execution configuration foundation has been added. Full-duplex coordination, scheduling, tool dispatch, and approval writes are not yet complete (see CURRENT REBUILD STATE).
...
- **Operation/execution configuration foundation (2026-08-17)**: `TinadecCore/DmaEA/Configuration/default-agent-runtime.toml` defines the annotated built-in baseline, including the `conversation`/`space` mode sets, `space.full_duplex`, `im`/`hub` aliases, a meeting-only `direct_user_output` rule, and default generation budgets (depth 2, 8 generated instances/run, 4 parallel workers, 2 active runs/session). When resolved, `AgentRuntimeConfigurationStore` validates the TOML and hot-reloads only valid in-process snapshots; invalid edits retain the prior snapshot. `agent_instances`, `agent_candidates`, and `runtime_profile_overrides` are relational projections. The legacy orchestrator still consumes `planning`/`execution` agent data and does not yet consume or freeze the resolved profile, apply workspace overrides, expose the diagnostic in readiness, create child agents, or implement full duplex.
...
- Storage + Gateway stubs: durable Core storage and a SQLite VectorStore foundation are present; no real model provider calls, no configured embedding route, and no full-duplex dual-layer runtime. Remaining stub endpoints allow Gateway/Desktop to function without crashes. See `TinadecCore/AGENTS.md` for details.
...
- **端口**: 48731
- **当前状态**: 已重建为基于 MAF 1.15.0 的 .NET 10 模块化单体，`TinadecCore/` 含 18 个源项目 + 3 个测试项目（详见 CURRENT REBUILD STATE）；项目/会话/消息 CRUD、会话运行列表与回放事件 SSE、旧式 DmaEA invocation（终态 invoke-stream SSE + orchestration 投影）及 TOML 配置基础已实现。后台全双工 coordinator、动态 spawn、记忆晋升、scheduling、tool dispatch 与 approval 写入尚未完成。
- **主要职责**: 
  - **唯一状态权威**: 管理会话、消息、项目、事件、审批等所有状态
  - **双层智能体编排**: 运营层 `operation`（主动协调、上下文、监督和候选建议）和执行层 `execution`（任务规划、worker 与证据交付）；`planning` 仅作为旧数据兼容输入
...
- **待补齐的能力**: 真实模型调用需要配置 API key（DevSeed 已建 `chat` 路由/provider/planner/executor，key 经 SecretStore 配置后即生效）、已配置的 embedding 路由、全双工 run coordinator、`context_revision`、动态 spawn/lineage、候选晋升与检索、Core-owned TinadecTools 进程、scheduling/tool dispatch/approval 写入；已就位的能力包括状态存储、harness 清单、就绪回执、回放事件、旧式 DmaEA invocation 与 TOML 配置读取基础
...
- Canonical dual-layer values are `operation` and `execution`. Accept `planning` only while migrating old inputs; new Core APIs, event payloads, persisted versions, and Desktop-visible values must use `operation`.
- `default-agent-runtime.toml` is an annotated built-in baseline. Its valid-only hot-reload store is a configuration foundation, not evidence that run freezing, workspace overrides, full-duplex coordination, spawn, memory promotion, or readiness exposure already work.
...
- **Mimosa git 门禁（本机 ZCode 插件，user-scope v1.0.3）**：`git commit` / `git push` 前会对整个仓库跑 L3 审计，任何 open high finding——即使与本次 diff 完全无关——都会强制拦截提交（graded 模式 high=deny）。在 Bash 命令里 export 的 `MIMOSA_*` 环境变量**无效**（钩子运行在 ZCode 宿主进程环境，不是 shell 子进程）；放行只能由用户在宿主/插件环境配置 `MIMOSA_NO_GIT_GATE=1` 或 `MIMOSA_GIT_GATE_MODE=warn`，或实际修复 findings。截至 2026-08-16 仓库存在历史遗留的 open high（Gateway/Tools 测试 fixture、沙箱命令构造与 Gateway 代理入口类问题，均非新分支引入；门禁拦截时会逐条列出文件与位置，以其输出为准）。修复它们是独立安全工作项，不要塞进无关合并。
