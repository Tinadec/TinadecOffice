...
**Last Updated By:** ZCode (merged PR 19 DmaEA dual-layer runtime + UI/StillREd feature-panel UI into main — conflict resolved and staged, commit pending Mimosa git-gate resolution; /init added the Mimosa commit-gate and AGENTS.md-merge-conflict gotchas below)
**Last Verified Commit:** f63f53c
**Branch:** main
...
## OVERVIEW
TinadecOffice is a Windows-first intelligent agent desktop workbench built around a universal agent harness. The target product combines a .NET 10 Core state/runtime service, an Elysia TypeScript Gateway, and an Electron + Vue desktop UI under one npm workspace root. The Core has been rebuilt as a MAF-based modular monolith; storage and read paths are in place, the DmaEA dual-layer invocation runtime is implemented, and scheduling, tool dispatch, and approval writes remain 501 stubs (see CURRENT REBUILD STATE).
...
- **DmaEA dual-layer runtime implemented (2026-08-05)**: `POST /api/v1/sessions/{id}/invoke-stream` appends the user message and runs planning → execution agents over the configured `chat` model route (`IChatResolver` port, OpenAI-compatible via `ChatClientAgent`), streaming a `kind:done`/`kind:error` SSE chunk and persisting run/task-snapshot/audit events. `GET .../orchestration`, `.../tool-executions`, and `.../task-nodes` return projections from replayed events. A new `DevSeed` (idempotent, Api composition via Runtime assembly) creates a `chat` route + OpenAI provider (no API key stored) + planner/executor agents on first startup; without a stored key the run fails cleanly (`run.failed` event, error chunk — never fake success). Scheduling, tool dispatch, and approval writes remain `501`.
...
- **端口**: 48731
- **当前状态**: 已重建为基于 MAF 1.15.0 的 .NET 10 模块化单体，`TinadecCore/` 含 18 个源项目 + 3 个测试项目（详见 CURRENT REBUILD STATE）；项目/会话/消息 CRUD、会话运行列表与回放事件 SSE、DmaEA 双层 invocation（invoke-stream SSE + orchestration 投影）已实现，scheduling、tool dispatch 与 approval 写入仍返回 501 stub
- **主要职责**: 
...
- **待补齐的能力**: 真实模型调用需要配置 API key（DevSeed 已建 `chat` 路由/provider/planner/executor，key 经 SecretStore 配置后即生效）、已配置的 embedding 路由、scheduling/tool dispatch/approval 写入；已就位的能力包括状态存储、harness 清单、就绪回执、回放事件与 DmaEA 双层 invocation 运行时
