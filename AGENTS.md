...
**Last Updated By:** Codex (made the Desktop Workbench a headless compatibility host and added an isolated Vue Vapor boundary)
...
- **Workbench Shell** (`apps/desktop/src/workbench/WorkbenchShell.vue`): 主窗口常驻、无界面的三槽位宿主；保留现有路由页面 UI，仅在后台选择页面预设和焦点
- **Workbench Layout** (`apps/desktop/src/workbench/types.ts`, `reducer.ts`, `constraints.ts`, `useWorkbenchLayout.ts`): 版本化纯布局、命令式变更、确定性约束、撤销/重做和作用域持久化
- **Card Registry / Surface Pool** (`apps/desktop/src/workbench/registry.ts`, `components/WorkbenchCanvas.vue`): 注册模块能力并按实例 ID 稳定挂载卡片
- **Panel Window** (`apps/desktop/electron/panelWindow.cjs`): 通用卡片分离/重新附加、发送方范围 IPC、窗口状态持久化和旧 `PanelType` 迁移
- **Workbench Layout Store** (`apps/desktop/electron/workbenchLayoutStore.cjs`): 在 Electron `userData` 下原子保存版本化布局
...
3. `docs/reference-project-map.md` - sibling-project reference map for VS Code, Traycer, Codex, t3code, OpenCode, OpenHarness, Open-ClaudeCode, openclaw, pi, DeepSeek-TUI, The Zeroth Docs, and Tinadice.
...
| Sibling project references | `docs/reference-project-map.md` | Maps VS Code, Traycer, Codex, t3code, OpenCode, OpenHarness, Open-ClaudeCode, openclaw, pi, DeepSeek-TUI, The Zeroth Docs, and Tinadice to TinadecOffice absorb/reject decisions. |
...
| Desktop Workbench | `apps/desktop/src/workbench`, `src/App.vue`, `src/router.ts` | Headless persistent three-slot shell; routes keep their existing page presentation while the engine selects a background layout and preserves visited instances. Renderer still talks only to Gateway. |
| Settings modules | `apps/desktop/src/pages/SettingsPage.vue`, `src/settings/settingsModuleRegistry.ts`, `src/settings/modules` | Locked navigation/content Workbench cards; all 12 content sections are async registry modules and visited modules remain mounted. |
...
| Workbench persistence / detached cards | `apps/desktop/electron/workbenchLayoutStore.cjs`, `electron/panelWindow.cjs`, `src/workbench/persistence.ts`, `src/pages/DetachedPanelPage.vue` | Atomic `userData/workbench-layout.json`, Web localStorage fallback, sender-scoped detached state in `userData/detached-cards.json`, legacy panel migration, and identity-preserving reattach. |
...
| Desktop `WorkbenchShell` | Headless module host | `apps/desktop/src/workbench/WorkbenchShell.vue` | Persistent host that keeps the existing RouterView page UI and its local state while routes select background layout/focus; its invisible `WorkbenchVaporRuntime.vue` is the isolated Vue Vapor boundary. |
| Desktop Workbench reducer | Layout authority | `apps/desktop/src/workbench/reducer.ts`, `constraints.ts`, `useWorkbenchLayout.ts` | Validates revisioned commands, returns inverse commands, resolves responsive degradation, and owns scoped persistence/history. |
| Desktop card registry | Module registry | `apps/desktop/src/workbench/registry.ts` | Defines card component, size, singleton, movement, close, detach, page, and serialization capabilities. |
...
- The main Desktop window mounts one persistent, headless `WorkbenchShell`. Main routes keep their present page UI while selecting `home` / `settings` / `market` / `code` / `debug` background layouts. Do not expose card chrome, grips, resizers, drop targets, tabs, layout menus, or any other new docking UI without an explicit product decision.
- Workbench layout mutation must go through the revisioned commands in `apps/desktop/src/workbench/types.ts` and `reducer.ts`. Do not mutate snapshots in card components or add a recursive docking tree.
- Cards are registry-owned modules and must remain keyed by stable instance ID while hidden, moved, or reordered. Explicit close is the only normal lifecycle that destroys a card instance.
- Workbench layout is local UI preference state, resolved workspace-page -> page -> global geometry -> built-in. It is not Core business state. Workspace identity is `ProjectDto.id`; Electron persists `userData/workbench-layout.json`, Web uses `tinadec-workbench-layout-v1`.
- Settings is a locked Workbench policy: fixed navigation/content cards, no foreign cards, movement, close, split, or detach; only navigation width is adjustable. New sections belong in the async settings registry and visited modules must retain their mounted state.
...
- The Desktop layout authority is now `apps/desktop/src/workbench/`; `SettingsPage.vue` remains the shared settings-domain state owner while section rendering lives in async modules under `src/settings/modules/`.
