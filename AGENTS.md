...
**Last Updated By:** Qoder (Desktop panel material system now re-maps material-aware `--surface-*` tokens via CSS custom property inheritance instead of per-component whitelist rules; spec in `docs/architecture.md` § Desktop Panel Material System)
**Last Verified Commit:** 938807b
**Branch:** main
...
| `usePanelStyles` | Composable | `apps/desktop/src/composables/usePanelStyles.ts` | Synchronously persists normalized global panel material settings in a detached module-lifetime storage scope; interior surfaces follow the material via `--surface-*` tokens defined in `styles.css` (spec: `docs/architecture.md` § Desktop Panel Material System). |
