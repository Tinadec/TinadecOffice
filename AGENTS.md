...
**Last Updated By:** Codex (Core rebuild reset)
**Last Verified Commit:** 57fb696
**Branch:** core'try
...
- On 2026-07-14 the entire legacy `src/` tree was intentionally removed: `src/TinadecCore`, `src/Tinadec.Contracts`, and `src/TinadecModel` no longer exist in the working tree.
- `TinadecCore/` exists at the repository root as an empty local rebuild target. Do not assume its final project/module layout until the replacement is designed.
- The former `gateway/` tree is currently present as `TinadecGateway/` due to a concurrent workspace rename. Root workspace scripts still name `gateway` and have not been repaired.
- Commit `57fb696` is the verified pre-deletion baseline. Read deleted implementation details with `git show 57fb696:<path>`; do not restore the old tree wholesale unless explicitly requested.
- `TinadecOffice.slnx`, the root Core npm scripts, and the Core/contract test projects still reference deleted paths. Full-stack restore, dev, build, and test are intentionally unavailable until the rebuild updates those contracts.
- Treat `docs/`, surviving Gateway/Desktop code, legacy tests, and Git history as design evidence. Preserve the Core/Tool/Desktop ownership boundaries while reconsidering implementation structure.
...
```
TinadecOffice/
├── TinadecCore/            # empty local rebuild target; final layout undecided
├── apps/desktop/          # Electron + Vue renderer, Debug Studio UI, and detachable panel windows
├── TinadecGateway/        # Elysia BFF/proxy; concurrently renamed from gateway/
├── TinadecTools/          # surviving approval-aware tool prototype host
├── TinadecTools.Generators/ # surviving tool registry source generator
├── tests/                 # TinadecTools tests plus legacy Core/contract requirements
├── docs/                  # product model, architecture, security, startup runbook
└── TinadecOffice.slnx      # stale until deleted Core/Contracts/Model references are rebuilt
```
...
The root `restore:dotnet`, `dev`, `build`, and full `test` commands are currently broken because `src/` was deleted while `package.json` and `TinadecOffice.slnx` still reference it. Do not report them as validation until the rebuild repairs those references.
