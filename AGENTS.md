...
**Last Updated By:** Hephaestus (settings center route split)
**Last Verified Commit:** e733325
**Branch:** setting/multi
...
| Tool layer UI | `apps/desktop/src/pages/settings/ToolsSettingsPage.vue`, `apps/desktop/src/pages/settings/toolSettings.ts`, `apps/desktop/src/toolCatalog.ts` | Desktop presents Core-governed Code-suite tools and Codex primitives. |
| Prompt context requirements | `apps/desktop/src/pages/settings/PromptContextSettingsPage.vue`, `apps/desktop/src/pages/settings/promptContextSettings.ts`, legacy tests, Git history | New Core must own storage/assembly; Desktop remains presentation only. |
...
- Desktop settings are nested hash routes under `apps/desktop/src/pages/settings/`; each domain owns a routed page and a focused state module.
