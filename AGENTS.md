...
**Last Updated By:** 自由对话闭环修复（审查 12 项发现：审批哨兵比较、网关必填 project_id、可空契约与侧栏可见性、migrate 复用 binder、SQLite 复合主键）
