...
**Last Updated By:** OpenCode (local Petdex desktop integration; Ponytail plugin)
**Last Verified Commit:** 55c770bbd12758c838d7197a8e58032b3af2a24f
**Branch:** desktop/pet
...
- `TinadecCore/` now contains 17 source projects, including `Tenancy` for external-principal, tenant, workspace, and membership isolation, plus the existing persistence, module, migration, runtime, and API projects.
...
| Local pet system | `apps/desktop/electron/petStore.cjs`, `apps/desktop/electron/petWindow.cjs`, `apps/desktop/src/components/PetPreview.vue`, `apps/desktop/src/pages/DesktopPetPage.vue` | Desktop-only Petdex v2 registry and transparent multi-window renderer. Files and enable state are local under Electron `userData/pets/`; Settings keeps downloaded pets above a 48-item lazy infinite-scroll market gallery, with previews served through a bounded local protocol loader. Never use Gateway/Core. |
