...
**Last Updated By:** opencode (Monet background color extraction: `@material/material-color-utilities` pinned 0.3.0, deterministic Wu+Score pipeline, `dynamic` accent + full-surface tinting, `tinadec:read-image-data-url` IPC)
**Last Verified Commit:** HEAD of DmaEA/MVP after Monet accent feature
**Branch:** DmaEA/MVP
...
- **模型获取与智能体模型策略收口 (2026-08-23, Core 155/Gateway 36/Desktop 247 green)**: 模型发现闭环——Desktop 触发 canonical `POST /api/v1/model-providers/{id}/models/refresh`，Core 持 SecretStore 密钥代理外部 `GET {base_url}/models`（密钥不出 Core），前端确认后把发现模型合并进 provider 配置并 PUT 持久化；Gateway 仅薄代理读取已写入状态。**智能体中心融合重设计 (同日)**：`agent_definitions` 扩 `system_prompt/description/enabled` 列（迁移 `202608230001_AgentDefinitionProfileFields`，Sqlite+Pg），DevSeed 基线智能体带岗位化提示词；Desktop 写路径全量版本化（`updateAgentDraft→publishAgent`，legacy `saveAgent` 客户端删除）；设置页智能体中心重设计为五子标签——agents/modes/prompts/evolution/runtime，`/agent-center` 独立路由与 `AgentCenterPage.vue`（2282 行）删除，`PromptContextPanel`/`PromptEngineeringPanel` 由 `PromptEngineeringMerged.vue` 吸收；inspector 分区卡含身份徽标(draft|published·vN)/模型策略(inherit|fixed|cli|acp)/工具与权限(审批升级路径说明+待处理授权计数)/版本历史(`listAgentVersions`)。已知遗留：Core 双注册 `GET /agents`（AgentConfiguration vs ControlPlane flat）仍待按 §14 收口。
- **外观 Monet 背景取色 (2026-08-24, Desktop 258 green)**: 强调色新增第 9 个值 `dynamic`（跟随背景）。`apps/desktop/src/lib/monetExtract.ts` 用官方 `@material/material-color-utilities`（**pinned 0.3.0**——0.4.0 的 ESM 构建有 extensionless import bug）实现确定性取色管线：QuantizerWu（直方图、无随机）+ 自算簇权重 + `Score.source()`。**刻意不用 `QuantizerCelebi`**：其 K-means 精修用裸 `Math.random()`，跨窗口各自提取会得到不同色板；Wu+Score 保证各窗口字节级一致，无需同步协议。`useDynamicPalette.ts` watch 图片背景设置静默提取（选图不自动切换主题色），结果缓存于 `tinadec-dynamic-palette`（vueuse 需显式 `StorageSerializers.object`——默认 null 会选中 any 序列化器导致读回原始 JSON 字符串）；换 video/html/none 冻结上次色板并显示提示行，重新选图恢复跟随。`applyAccentColor('dynamic')` 写 44 个 CSS 变量（全表面着色：`--bg-*`/中性 `--border-*`/`--text-primary/secondary`/强调族/`--bg-*-rgb` 三元组），切回预设时按 `DYNAMIC_VAR_NAMES` 逐个移除；status/error 语义实体色不动（styles.css 契约）。本地图经新 IPC `tinadec:read-image-data-url` 读为 data URL（dev 渲染层是 http origin，canvas 直读 file:// 必被 tainted-canvas 拦截；main.cjs 有扩展名白名单 + 50MB 上限）。Monaco 用内置 vs-dark/vs 不消费这些变量，无需挂钩。
