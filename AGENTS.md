...
**Last Updated By:** dev 残留端口守卫（`scripts/dev-port-guard.mjs` + concurrently `--kill-others`）+ 桌面端 ELECTRON_RUN_AS_NODE 防护 + OfficeAgentPack 版本号修正（0.2.3 → 0.2.4，digest 与 `metadata.version` 同步）
