...
**Last Verified Commit:** 4b524615ebb2187724fa64705f5c771c4d8aaa16
**Branch:** main
...
- `TinadecTools` 的 Git 历史工具 `git_log_list`、`git_log_detail`、`git_file_history` 都需要审批。它们只读取工作区内 Git 仓库；`git_file_history.path` 必须解析到仓库内且不得穿越 symbolic link 或 junction。补丁输出受 `max_patch_bytes` 预算限制，超限时保留提交/文件元数据并返回 `truncated: true` 与 `truncation_reason`。
