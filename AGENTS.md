...
**Last Updated By:** Claude (added Terminal.Gui TUI client welcome page under `apps/tui/` — independent `TinadecOffice.Tui.slnx`, `dev:tui` script, TUI layer boundary notes)
**Last Verified Commit:** 234b190
**Branch:** tui
...
### TUI层 (终端呈现层)
- **技术栈**: .NET 10 C# + Terminal.Gui 2.4.17（v2 实例式 API）
- **位置**: `apps/tui/`，**独立 `TinadecOffice.Tui.slnx`**（仿 `TinadecCore/TinadecCore.slnx`，不注册进根 `TinadecOffice.slnx`）
- **端口**: 无固定端口；默认连 Gateway `http://127.0.0.1:48730`（`TINADEC_GATEWAY_URL` 可覆盖）
- **主要职责**:
  - 纯终端环境下的呈现面（欢迎页起步，后续按桌面功能逐屏扩展）
  - 通过 Gateway 访问 `/api/v1/*`，与 Desktop 同一契约
- **设计原则**: 与 Desktop/Web 一样只调 Gateway、不存业务状态；Windows 优先（ANSI 驱动，Windows Terminal TrueColor）
- **运行**: `npm run dev:tui`（经 `scripts/setup-dotnet-env.ps1` 清除 `Version`/`Ice-Version`）
...
### 核心设计原则
1. **Core是唯一状态权威**: Gateway/Desktop/TUI 不存储任何状态
2. **Desktop/TUI 只调Gateway**: 不直接调用Core
...
├── apps/tui/              # Terminal.Gui TUI client (independent TinadecOffice.Tui.slnx; welcome page first)
├── apps/web/              # Browser client reusing desktop renderer via alias (no UI code)
...
| TUI client | `apps/tui/` (`Program.cs`, `WelcomePage.cs`, `AppTheme.cs`, `GatewayClient.cs`) | Terminal.Gui 2.4.17 v2 API; independent slnx; `TINADEC_GATEWAY_URL` override; talks to Gateway `/api/v1/health`. |
...
- Reserved dev ports remain Gateway `48730`, Core `48731`, Vite `5173`. TUI defaults to Gateway `http://127.0.0.1:48730`, overridable via `TINADEC_GATEWAY_URL`.
...
- Do not make Desktop/TUI call Core directly; Desktop/TUI call Gateway.
- Do not store session state, approvals, model routes, or provider lifecycle state in Gateway/Desktop/TUI; Core owns it.
...
# Start TUI client only (independent apps/tui/TinadecOffice.Tui.slnx; needs a real terminal)
npm run dev:tui
...
TUI client sub-solution only (`apps/tui/TinadecOffice.Tui.slnx`):
