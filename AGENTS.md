...
- `TinadecCore/` now contains 16 source projects (Contracts, Abstractions, Persistence, SQLite/PostgreSQL storage migrations, Strategies F#, DmaEA, Models, Context, Prompts, Memory, Skills, LoopGuard, Lifecycle, Runtime, Api) and 3 test projects (Architecture, AgentFramework, Api).
- Shared database abstraction lives in `TinadecCore/Persistence`: EF Core LINQ surface, default **SQLite** local file, optional **PostgreSQL** via config. The storage foundation has exactly four relational index tables: `projects`, `sessions`, `runs`, and `event_index`; full histories, task graphs, events, and artifacts remain Core-owned files under `data/`.
...
- Storage + Gateway stubs: durable Core storage is present; no real provider calls, no vector store, and no full dual-layer runtime. Remaining stub endpoints allow Gateway/Desktop to function without crashes. See `TinadecCore/AGENTS.md` for details.
...
```
TinadecOffice/
├── TinadecCore/            # MAF-based modular monolith (16 source + 3 test projects)
...
└── TinadecOffice.slnx      # Root solution (Core + Tools + Tools.Tests; includes Persistence)
```
