...
**Last Updated By:** Qoder (added `apps/manger` application installer suite: Installer.Core engine + Terminal.Gui TUI + MewUI NativeAOT GUI, own `Tinadec.Installer.slnx`)
...
├── apps/manger/           # Standalone application installer suite (Core engine + Terminal.Gui TUI + MewUI GUI), own Tinadec.Installer.slnx
...
| Application installer | `apps/manger/` | Standalone GitHub-Releases installer (not part of the root solution). `Installer.Core` owns download/extract/rollback + dependency/version logic; `Installer.TUI` is a Terminal.Gui v2 dashboard + CLI (`dotnet run -- help`); `Installer.GUI` is a MewUI (Aprillz.MewUI) NativeAOT window app. Storage lives under `%LOCALAPPDATA%/Tinadec` (`~/.tinadec` on Unix). Build via `apps/manger/Tinadec.Installer.slnx`. |
