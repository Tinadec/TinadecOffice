...
- `TinadecTools` 原型里的静态工具只写 `[ToolFunction(...)]`；注册器由 source generator 生成，非静态工具则通过 `ToolHandlerBase<TArgs, TResult>` 或显式 `ToolRegistry.Register(...)` 接入。
