...
export default defineSettingsModule('SettingsModelModule')
</script>

<template>
<div class="settings-module" data-settings-module="model">
          ...
          <section v-if="modelCenterSection === 'cli'" class="center-resource-section">
            ...
            <div class="center-resource-list">
              <article v-for="runtime in modelCenterOverview?.cli_runtimes ?? []" :key="runtime.runtime_id" class="center-resource-list-row">
                ...
                <UiButton
                  v-if="providers.find((provider: any) => provider.id === runtime.provider_instance_id)"
                  variant="outline"
                  size="sm"
                  @click="openEditModal(providers.find((provider: any) => provider.id === runtime.provider_instance_id)!)"
                >
                  <Settings2 :size="14" />
