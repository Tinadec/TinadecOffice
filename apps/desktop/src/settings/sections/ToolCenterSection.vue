 ...
 *
 * Self-contained: loads the harness manifest, tool-layer readiness receipt,
 * and tool discovery results directly from Core/Gateway on mount. All facts
...
function projectTemplatesFromResultSafe(result: CodeToolExecuteResultDto | null | undefined): ProjectTemplateSummary[] {
  return projectTemplatesFromResult(result)
}
...
async function loadAgentCenter(): Promise<void> {
  // Kept name for template parity with the previous refresh button.
  loading.value = true
  try {
    const [readiness, manifest] = await Promise.all([
      api.getToolLayerReadiness().catch(() => null),
      api.getHarnessManifest().catch(() => null),
    ])
    if (readiness) toolLayerReadiness.value = readiness as ToolLayerReadinessReceiptDto
    if (manifest) {
