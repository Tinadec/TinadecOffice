...
**Last Updated By:** Claude (fixed Ctrl+R reload-blank: de-vapored AppSplash so the root splash-exit Transition is classic-around-classic, installed global renderer error fallback in main.ts, hardened workbench layout hydrate/restore with a catch + unload flush)
