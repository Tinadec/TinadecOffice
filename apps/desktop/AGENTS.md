...
| Unified notifications | `src/composables/useNotifications.ts`, `src/components/NotificationIslandHost.vue`, `src/components/ConfirmationDialog.vue`, `src/App.vue` | Window-local singleton queue: up to three compact islands, expandable overflow/details, persistent banners, and a separate FIFO native confirmation dialog. |
...
- `useNotifications()` owns transient operation feedback, persistent cross-page banners, and UI confirmations. Keep initial/context load errors, field validation, diagnostics, Git conflicts, and approval lifecycle state inline. Core approvals and Tool `confirm_*` remain authoritative and must never be replaced by a notification or UI confirmation.
- Notification state is independent per renderer window. Standard windows use the title-bar corridor; detached and Debug Studio windows place islands below their occupied title bars; the transparent pet route never mounts notification hosts.
