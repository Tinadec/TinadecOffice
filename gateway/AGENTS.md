...
- `git_worktree_manager` retains read and migrated mutation actions as compatibility adapters that forward to approval-gated TinadecTools tools. Direct Git mutation ids require a Core-verified `kind=git` approval and explicit confirmation; pull is fast-forward-only, merge supports start/continue/abort, and rebase supports start/continue/abort/skip. Conflict resolution remains pending Tool-layer migration.
