workspace "TinadecOffice 架构（当前态）" "证据来源：仓库源码、solution、配置与 docs。生成日期 2026-08-27，基线 commit f9c44c0。配套证据索引见 tinadecoffice.evidence.md。" {
    ...
    model {
        ...
        app = softwareSystem "TinadecApp" "面向场景的交互客户端与随 App 分发的 Agent Pack 制品。" {
            ...
            officePack = container "OfficeAgentPack 制品" "静态携带 tinadec.office.agent-pack@0.2.4（14 Agent + 5 条提示词管线 + 7 Mode，治理角色 tool_scope 为空，worker 绑定角色管线）与硬编码 RFC8785 摘要。" "apps/desktop/src/agentPacks/OfficeAgentPack"
        }
