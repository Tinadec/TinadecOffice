...
17. **`check:drift` 之外还有一道门覆盖不到卡片，缺键因此能出货**：i18n parity 门（`apps/desktop/src/locales/i18nParity.test.ts:33-54` 的 `panelSources`）只登记 `apps/desktop/src/**`，`apps/TinadecUI/src/components/cards/**` 一条都没进——实测后果就在市场页：`MarketDetailCard.vue:159` 用 `t('market.required')`/`t('market.secret')`，而两个语言包里只有 `envRequired`/`envSecret`（`en.ts:1182-1183`），于是英文和中文界面上都会显示点号键名。同一条门的两分支叶子集相等检查也帮不上，因为两个包**都**没这两个键。修法不是补两个键就完事：把 `apps/TinadecUI` 的卡片纳入 `panelSources`，让"卡片里写了的键必须在双语里存在"变成机器约束（任务 #10 的可访问性/令牌契约扩面同族）。
