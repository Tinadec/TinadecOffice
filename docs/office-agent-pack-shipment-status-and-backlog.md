...
- **Pack v0.2.1（2026-08-31 双层权限收口）**：五个非 steward 治理角色 `tool_scope` 由 `["*"]` 收口为 `[]`，与 Core `CoreAuthorizationContextResolver` 新增的 operation 层零工具策略一致；**该策略已于 2026-09-17 全局撤下**（治理层可持工具，改为靠包显式声明工具面 + 写授权 + 逐次审批三道防线），此条仅作历史记录；digest `8110547a…962e`。`worker.general` 的通配按产品决定保留（`conversation.vibe` 唯一可写兜底 worker），且 `AgentInstanceService.SpawnAsync` 已禁止派生实例携带通配，通配不会再落到任何实例上。
