...
* 权限收口（2026-08-31）：operation 层零工具（`operation_layer_cannot_invoke_tools`）+ `FreezeToolManifest` 仅并 execution agents + `SpawnAsync` 拒通配 `*` + 实例授权 ∩ 冻结清单（`IFrozenToolManifestCatalog`）。**该条中的「operation 层零工具」已于 2026-09-17 全局撤下**（产品决策：治理层可持工具，见 `AGENTS.md` 的 SOLO DISPATCH 段）；其余各条仍有效。
