...
> 本文是既有实现计划和阶段记录。DmaEA 的正式名称、双层职责、权限模型、演化机制与后续路线图以 [TinadecCore 产品定义与 DmaEA 架构基线](tinadec-core-product-definition.zh-CN.md) 为准。

## 核心理念

- 正式采用“治理层 + 执行层”：治理层（机器值 `operation`）负责会议入口、全局协调、上下文、监督和经验提炼；执行层负责任务规划、动态创建 worker、工具执行和证据交付。
