...
public sealed class AgentRuntimeBindings : Migration
{
    protected override void Up(MigrationBuilder m) => m.Sql("""
        create table if not exists agent_runtime_bindings (
            ...
            updated_at timestamptz not null, updated_by_principal_id text null
        );
...
{
    protected override void Up(MigrationBuilder m) => m.Sql(
        "alter table agent_runtime_bindings add column if not exists route_purpose text null;");

    protected override void Down(MigrationBuilder m) => m.Sql(
        "alter table agent_runtime_bindings drop column if exists route_purpose;");
}
