...
public sealed class ControlPlaneService
{
    ...
    { if (!input.TryGetProperty("provider_instance_id", out var provider) || !Guid.TryParse(provider.GetString(), out var providerId)) return Results.BadRequest(new { message = "provider_instance_id is required" }); await using var db = await _models.CreateDbContextAsync(ct); var row = await db.Routes.SingleOrDefaultAsync(x => x.Purpose == purpose && x.TenantId == Tenant.TenantId && x.WorkspaceId == Tenant.WorkspaceId && x.DeletedAt == null, ct); var now = DateTimeOffset.UtcNow; if (row != null && !Matches(row.Revision, ifMatch)) return Results.StatusCode(412); if (row == null) { row = new ModelRouteRecord { Id = Guid.NewGuid(), TenantId = Tenant.TenantId, WorkspaceId = Tenant.WorkspaceId, Purpose = purpose, CreatedByPrincipalId = Tenant.PrincipalId, CreatedAt = now }; db.Routes.Add(row); } else { // Legacy rows may carry revision 0 while a version already exists; the
          // unique (route, version) index turns that into an INSERT collision. Derive
          // the next version from durable rows, not from the mutable counter.
          var maxVersion = await db.RouteVersions.Where(v => v.RouteId == row.Id).MaxAsync(v => (int?)v.Version, ct); if (maxVersion.HasValue && maxVersion.Value > (int)row.Revision) row.Revision = maxVersion.Value; } var version = new ModelRouteVersionRecord { Id = Guid.NewGuid(), RouteId = row.Id, Version = (int)row.Revision + 1, ProviderId = providerId, Model = input.TryGetProperty("model", out var m) ? m.GetString() : null, CreatedByPrincipalId = Tenant.PrincipalId, CreatedAt = now }; row.Revision++; row.CurrentVersionId = version.Id; row.UpdatedByPrincipalId = Tenant.PrincipalId; row.UpdatedAt = now; db.RouteVersions.Add(version); await db.SaveChangesAsync(ct); return Results.Ok(new { purpose = row.Purpose, provider_instance_id = version.ProviderId, model = version.Model, revision = row.Revision, updated_at = row.UpdatedAt }); }
