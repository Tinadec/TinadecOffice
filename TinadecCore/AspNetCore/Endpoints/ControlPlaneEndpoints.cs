...
{
    // Provider/route mutations are conditional updates: If-Match carries the row revision
    // (428 when missing, 412 when stale). A successful write echoes the new revision as ETag.
    private static void SetRevisionEtag(HttpResponse response, long revision) => response.Headers.ETag = $"\"{revision}\"";
    ...
    public static IEndpointRouteBuilder MapControlPlaneEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        app.MapPost("/api/v1/model-providers", async (HttpRequest request, HttpResponse response, ControlPlaneService service, CancellationToken ct) =>
        {
            var result = await service.SaveProvider(await JsonSerializer.DeserializeAsync<JsonElement>(request.Body, cancellationToken: ct), null, request.Headers.IfMatch.FirstOrDefault(), ct);
            await WithEtagAsync(response, result, ct);
            return result;
        });
        app.MapPut("/api/v1/model-providers/{id:guid}", async (Guid id, HttpRequest request, HttpResponse response, ControlPlaneService service, CancellationToken ct) =>
        {
            var force = ParseForce(request);
            var result = await service.SaveProvider(await JsonSerializer.DeserializeAsync<JsonElement>(request.Body, cancellationToken: ct), id, request.Headers.IfMatch.FirstOrDefault(), ct, force);
            await WithEtagAsync(response, result, ct);
            return result;
        });
        ...
        app.MapPut("/api/v1/model-routes/{purpose}", async (string purpose, ModelRouteWriteRequestDto request, HttpRequest httpRequest, HttpResponse response, ControlPlaneService service, CancellationToken ct) =>
        {
            var result = await service.SaveRoute(purpose, request, httpRequest.Headers.IfMatch.FirstOrDefault(), ct);
            await WithEtagAsync(response, result, ct);
            return result;
        });
        ...
        app.MapPost("/api/v1/approvals/pre-authorizations", (PreAuthorizationRequestDto input, ControlPlaneService service, CancellationToken ct) => service.CreatePreAuthorization(input, ct));
    ...
    private static bool ParseForce(HttpRequest request)
        => request.Query.ContainsKey("force") &&
           (request.Query["force"].Count == 0 || !bool.TryParse(request.Query["force"].ToString(), out var parsed) || parsed);

    // The service returns anonymous objects whose only reliable revision carrier is the
    // "revision" JSON field; echo it as an ETag so clients can chain conditional updates.
    private static async Task WithEtagAsync(HttpResponse response, IResult result, CancellationToken ct)
    {
        if (result is not Microsoft.AspNetCore.Http.HttpResults.Ok<object> ok || ok.Value is null) return;
        var revisionProperty = ok.Value.GetType().GetProperty("revision");
        if (revisionProperty?.GetValue(ok.Value) is not long revision) return;
